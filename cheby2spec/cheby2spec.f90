...
!----------------------------------------------------------------------
    ...
    do i=1,epoints
       ...
       if (lpade) then
          write(unit,'(ES15.6,4(2x,ES15.6))') &
               (e+eshift)*convfac,spec(4),spec(1),spec(3),spec(0)
       else
          write(unit,'(ES15.6,3(2x,ES15.6))') &
               (e+eshift)*convfac,spec(1),spec(3),spec(0)
       endif
...
!######################################################################
  ...
  subroutine write_header(unit)
    ...
    integer          :: unit,i
    character(len=1) :: acomp(3)

    acomp=(/'x','y','z'/)
    
    if (lpade) then

       write(unit,'(84a)') '#',('+',i=1,83)
       write(unit,'(a)') '# Dipole component: '//acomp(ic)
       write(unit,'(84a)') '#',('+',i=1,83)
